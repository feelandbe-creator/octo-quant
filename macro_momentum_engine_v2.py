...
# --- 1. 초정밀 개별 다운로드 및 병합 엔진 (V2.3 핵심 업데이트) ---
...
def fetch_comprehensive_market_data(tickers, start_date, end_date):
    ...
    for ticker in tickers:
        try:
            # 개별 종목씩 정밀하게 다운로드 (묶음 다운로드 시 발생하는 누락 원천 차단)
            ...
            # 시리즈의 이름을 해당 티커로 지정하여 데이터프레임에 안전하게 병합
            compiled_data[ticker] = price_series
            
        except Exception:
            continue
            
    # 데이터 병합 후, 한국/미국 휴장일 불일치로 인한 빈칸을 이전 날짜 데이터로 채움
...
def find_top_historical_matches(df, macro_tickers, target_stock, window_size, top_n=3):
    # 타겟 주식이 데이터에 없다면 에러를 방지하고 빈 값 반환
    if target_stock not in df.columns:
        return [], {}
        
    # 실제 다운로드에 성공한 매크로 지표만 필터링
...
# [핵심 방어] 사용자 입력 시 발생하는 눈에 보이지 않는 공백(.strip()) 자동 제거
...
if st.sidebar.button("⚙️ 고정밀 시뮬레이션 개시"):
    with st.spinner('고정밀 매크로 데이터 수집 및 다차원 DTW 행렬 연산을 수행 중입니다. 잠시만 기다려 주십시오...'):
        ...
        # [핵심 방어] 데이터 수집 실패 시 앱이 다운되지 않고 원인을 알려주는 안전장치
        if target_stock not in df.columns:
